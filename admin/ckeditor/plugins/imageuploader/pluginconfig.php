...
if($stmt = $db->prepare("SELECT upload_folder_root.name FROM menu INNER JOIN upload_folder_root ON upload_folder_root.id = menu.upload_folder_id WHERE menu.name = ?"))
{

    $stmt->bind_param("s", $_SESSION["menu"]);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
    $uploadfoldername = $row["name"]; 
    //echo "uploadfoldername: ".$uploadfoldername;

    $useruploadfolder = $config->SITE_ROOT ."/".$config->UPLOAD_FOLDER ."/".(empty($_SESSION["linkedTo"])?$config->LINKED_TO_DEFAULT:$_SESSION["linkedTo"])."/".$uploadfoldername."/";

    $useruploadpath = $config->ABSOLUTE_UPLOAD_FOLDER."/".(empty($_SESSION["linkedTo"])?$config->LINKED_TO_DEFAULT:$_SESSION["linkedTo"])."/".$uploadfoldername."/";
